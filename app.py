...
def query_db_with_commit(query, args=()):
    ...
    - cur.close():
        Fecha a conexão
    """
    conn = get_db_connection()  # Obtém a conexão com o banco
    cur = conn.cursor()  # Cria o cursor
    cur.execute(query, args)  # Executa a consulta
    
    linhas_afetadas = cur.rowcount  # Obtém o número de linhas afetadas
...
def set_usuario(data): #Alterado de setUsuario para set_usuario
    ...
    linhas_modificadas = query_db_with_commit('''INSERT INTO tb_usuario(nome, nascimento)
                        values (%s, %s)''',(data.get('nome'), data.get('nascimento')))
...
def update_usuario(user_id, data): #Alterado de updateUsuario para update_usuario
    ...
    """
    linhas_modificadas = query_db_with_commit( '''UPDATE tb_usuario SET nome = %s, nascimento
                       = %s WHERE id = %s''',(data.get('nome'),data.get('nascimento'), user_id))
    return linhas_modificadas
...
def delete_usuario_fisico(user_id): #Alterado de deleteUsuario para delete_usuario
    ...
    """
    linhas_modificadas = query_db_with_commit( 'DELETE FROM tb_usuario WHERE id = %s',(user_id,))
    return linhas_modificadas
...
def delete_usuario_logico(user_id): #Alterado de deleteUsuario para delete_usuario
    ...
    linhas_modificadas = query_db_with_commit( '''UPDATE tb_usuario SET
                       deleted_at = CURRENT_TIMESTAMP WHERE id = %s''',(user_id,))
    return linhas_modificadas
